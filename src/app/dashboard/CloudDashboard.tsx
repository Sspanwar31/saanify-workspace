            ...
            >
              ...
              <div className="bg-gray-900 dark:bg-black rounded-lg p-4 h-96 overflow-y-auto font-mono text-xs text-green-400 border border-gray-700">
                <div className="space-y-1">
                  <div>[{new Date().toISOString()}] INFO: Saanify Cloud Dashboard connected</div>
                  <div>[{new Date().toISOString()}] INFO: Database schema synchronized</div>
                  <div>[{new Date().toISOString()}] INFO: {logFilter.toUpperCase()} operations ready</div>
                  <div>[{new Date().toISOString()}] DEBUG: Processing user authentication</div>
                  <div>[{new Date().toISOString()}] INFO: Auth token validated successfully</div>
                  <div>[{new Date().toISOString()}] DEBUG: Checking storage permissions</div>
                  <div>[{new Date().toISOString()}] INFO: Storage access granted</div>
                  <div>[{new Date().toISOString()}] INFO: All systems operational</div>
                  <motion.div
                    animate={{ opacity: [0.5, 1] }}
                    transition={{ duration: 1, repeat: Infinity }}
                  >
                    <div>[{new Date().toISOString()}] INFO: Live monitoring active...</div>
                  </motion.div>
                </div>
              </div>
            </motion.div>
                    ...
                    <CardContent className="p-0">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="font-semibold text-gray-900 dark:text-white text-sm">{secret.name}</h4>
